...
package icox.cjy.seeobject.views;
...
//	@Override
	public boolean onTouchs(View view, MotionEvent me) {
	...
	private void startCurl(int page) {
		...
		case CURL_LEFT: {
			...
			if (mCurrentIndex > 1) {
				Log.e("mytest","mPageLeft mCurrentIndex = "+mCurrentIndex);
	...
	private void updatePages() {
		...
		if (rightIdx >= 0 && rightIdx < mPageProvider.getPageCount()) {
			Log.e("mytest","右边 rightIdx = "+rightIdx);
		...
		if (leftIdx >= 0 && leftIdx < mPageProvider.getPageCount()) {
			Log.e("mytest","左边 leftIdx = "+leftIdx);
